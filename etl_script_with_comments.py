...
def extract():
    ...
    cursor.execute("""
        ...
        (SELECT GROUP_CONCAT(actor_id) FROM (SELECT actor_id FROM movie_actors WHERE movie_id = movies.id)), 
        max(writer, writers)
        FROM movies
    """)
...
def transform(__actors, __writers, __raw_data):
    ...
    for movie_info in __raw_data:
        ...
        document = {
            ...
            "director": director,
            "actors": [{"id": actor[0], "name": actor[1]} for actor in set(actors_list) if actor[1]],
            "writers": [{"id": writer[0], "name": writer[1]} for writer in set(writers_list) if writer[1]]
        }
